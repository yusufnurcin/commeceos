...
export default async function CatalogProductsPage() {
  ...
  return <AdminShell navigation={navigationManifest} principal={session.principal}><PageHeader actions={<Link href="/catalog/categories">Kategoriler</Link>} item={{ ...item, status: "runtime-ready", description: "Commerce OS Core ürün kayıtlarını ve moderasyon kararlarını yönetin. Medusa sync opsiyonel provider kuyruğudur." }} principal={session.principal} /><CatalogProductsPanel initialProducts={productPayload.products ?? []} initialSyncJobs={jobPayload.syncJobs ?? []} medusaHealth={productPayload.medusaHealth} module={productPayload.module} /></AdminShell>;
}
