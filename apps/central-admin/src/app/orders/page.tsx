...
export default async function OrdersPage() {
  ...
  return <AdminShell navigation={navigationManifest} principal={session.principal}><PageHeader actions={<><Link href="/orders/returns">İadeler</Link><Link href="/orders/refunds">Refundlar</Link></>} item={{ ...item, status: "runtime-ready", description: "Commerce OS Core sipariş kayıtlarını ve iç operasyon durumlarını yönetin. Medusa order sync opsiyonel provider kuyruğudur." }} principal={session.principal} /><OrderOperationsPanel initialOrders={orderPayload.orders ?? []} initialSyncJobs={jobPayload.syncJobs ?? []} medusaHealth={orderPayload.medusaHealth} module={orderPayload.module} /></AdminShell>;
}
